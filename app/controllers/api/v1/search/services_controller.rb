...
class Api::V1::Search::ServicesController < Api::V1::Search::ApplicationController
  ...
  def serialize_offers(offers)
    ...
    offers
      .with_highlights
      .group_by { |offer, _| offer.service_id }
      .to_h
  ...
  def facets(from_search)
    ...
    propagate_counts_upwards =
      lambda do |nodes|
        nodes.each do |node|
          ...
          child_counts = kids.map { |child| child[:count].to_i }
          node[:count] = child_counts.sum if child_counts.any?(&:positive?)
        end
        nodes
      end
    ...
    category_counts = category_buckets.to_h { |bucket| [bucket["key"].to_i, bucket["doc_count"].to_i] }
    sd_counts = sd_buckets.to_h { |bucket| [bucket["key"].to_i, bucket["doc_count"].to_i] }
    ...
    provider_counts = provider_buckets.to_h { |bucket| [bucket["key"].to_i, bucket["doc_count"].to_i] }
    providers =
      Provider.pluck(:id, :name, :pid).map { |id, name, pid| { name: name, eid: pid, count: provider_counts[id] || 0 } }
    providers.sort_by! { |hash| [-hash[:count], hash[:name].to_s.downcase] }

    jurisdiction_buckets = extract_buckets.call(aggs, :jurisdiction)
    jurisdiction_counts = jurisdiction_buckets.to_h { |bucket| [bucket["key"].to_s, bucket["doc_count"].to_i] }
    jurisdictions =
      Vocabulary::Jurisdiction
        .pluck(:name, :eid)
        .map { |name, eid| { name: name, eid: eid, count: jurisdiction_counts[eid] || 0 } }
    jurisdictions.sort_by! { |hash| [-hash[:count], hash[:name].to_s.downcase] }
    ...
    rating_counts = rating_buckets.to_h { |bucket| [bucket["key"].to_s, bucket["doc_count"].to_i] }
    ratings =
    ...
    order_type_counts = order_type_buckets.to_h { |bucket| [bucket["key"].to_s, bucket["doc_count"].to_i] }
    order_types =
