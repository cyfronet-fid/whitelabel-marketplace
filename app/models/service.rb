...
class Service < ApplicationRecord
  ...
  def owned_by?(user)
    (resource_organisation && resource_organisation.data_administrators&.map(&:user_id)&.include?(user.id)) ||
      (catalogue && catalogue.data_administrators&.map(&:user_id)&.include?(user.id))
  end
