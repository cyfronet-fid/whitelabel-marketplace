...
class Datasource::PcCreateOrUpdate
  ...
  def initialize(eosc_registry_datasource, status)
    ...
    @eid = eosc_registry_datasource["id"] || eosc_registry_datasource.dig("service", "id")
    @service_id = eosc_registry_datasource["serviceId"] || eosc_registry_datasource.dig("service", "serviceId")
    @mp_datasource =
      Service.joins(:sources).find_by("service_sources.source_type": @source_type, "service_sources.eid": @eid) ||
        Service.joins(:sources).find_by("service_sources.source_type": @source_type, "service_sources.eid": @service_id)
