...
class StripServiceToV6 < ActiveRecord::Migration[7.2]
  ...
  def up
    add_column :services, :publishing_date, :date, if_not_exists: true
    add_column :services, :resource_type, :string, if_not_exists: true
    add_column :services, :urls, :string, array: true, default: [], if_not_exists: true
    ...
    execute "DELETE FROM contacts WHERE contactable_type = 'Service' AND type IN ('MainContact', 'PublicContact')"
