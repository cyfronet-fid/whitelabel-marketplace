...
class AddPublicContactEmailsToProviders < ActiveRecord::Migration[7.2]
  def up
    add_column :providers, :public_contact_emails, :string, array: true, default: [], if_not_exists: true

    say_with_time "Backfilling provider public_contact_emails from public_contacts" do
  ...
  def down
    remove_column :providers, :public_contact_emails, if_exists: true
  end
end
